...
- **Minimum text sizes** — nothing below 11px in any canvas drawing. Tick values: 11px. Axis titles: 12px. Legends: 14px. Panel titles (CSS): 13px. These are native canvas pixels — if CSS-scaled up, they appear larger (good). If CSS-scaled down (embedded), they must still be readable.
- **Text opacity** — 0.6 minimum (`rgba(255,255,255,0.6)`). Never use 0.35 — too faint.
